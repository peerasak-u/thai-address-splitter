...
describe('Landline phone support', () => {
    const input = 'นายสมชาย ใจดี 02-123-4567 123 ถ.ปทุมวัน ปทุมวัน กรุงเทพ 10330';
    const result = Splitter.split(input);
    
    it('should extract Bangkok landline', () => {
        assert.equal(result.phone, '021234567');
    });
});

describe('International phone support', () => {
    const input = 'นายธนกฤต พลอย +66-81-234-5678 บ้านเลขที่ 55 ซ.ลาดพร้าว ลาดพร้าว กรุงเทพ 10230';
    const result = Splitter.split(input);
    
    it('should extract +66 format', () => {
        assert.equal(result.phone, '66812345678');
    });
});

describe('Military title support', () => {
    const input = 'พลเอกสุรยุทธ์ จุฑานนท์ 081-234-5678 1 ถ.สามเสนใน พญาไท กรุงเทพ 10400';
    const result = Splitter.split(input);
    
    it('should extract military title', () => {
        assert.equal(result.name, 'พลเอกสุรยุทธ์ จุฑานนท์');
    });
});

describe('Police title support', () => {
    const input = 'พลตำรวจประยุทธ์ เดชะคุณ 081-111-2233 1 ถ.ปทุมวัน กรุงเทพ 10330';
    const result = Splitter.split(input);
    
    it('should extract police title', () => {
        assert.equal(result.name, 'พลตำรวจประยุทธ์ เดชะคุณ');
    });
});

describe('Religious title support', () => {
    const input = 'พระสุรชัย วัดปทุมธานี 081-234-5678 555 ม.5 ต.ลาดพร้าว ลาดพร้าว กรุงเทพ 10230';
    const result = Splitter.split(input);
    
    it('should extract พระ title', () => {
        assert.equal(result.name, 'พระสุรชัย วัดปทุมธานี');
    });
});

describe('Academic title support', () => {
    const input = 'อาจารย์สมชาย ศรีสุข 02-333-4444 88 ถ.สามเสนใน พญาไท กรุงเทพ 10400';
    const result = Splitter.split(input);
    
    it('should extract อาจารย์ title', () => {
        assert.equal(result.name, 'อาจารย์สมชาย ศรีสุข');
    });
});